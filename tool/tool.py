...
def feibiao_login(username, password):
    ...
    # 获取验证码
    driver.save_screenshot('./verification_code/FeiBiaoLogin.png')
    imgelement = driver.find_element(By.XPATH, '//*[@id="img_captcha"]')  # 定位验证码
    location = imgelement.location  # 获取验证码x,y轴坐标
    size = imgelement.size  # 获取验证码的长宽
    rangle = (int(location['x']), int(location['y']), int(location['x'] + size['width']),
              int(location['y'] + size['height']))  # 写成我们需要截取的位置坐标
    i = Image.open("./verification_code/FeiBiaoLogin.png")  # 打开截图
    frame4 = i.crop(rangle)  # 使用Image的crop函数，从截图中再次截取我们需要的区域
    frame4.save('./verification_code/code.png')  # 保存我们接下来的验证码图片 进行打码

    # 识别验证码
    sdk = muggle_ocr.SDK(model_type=muggle_ocr.ModelType.Captcha)
    with open(r"./verification_code/code.png", "rb") as f:
        img = f.read()
